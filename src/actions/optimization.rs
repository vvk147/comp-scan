...
pub fn kill_top_memory_action() -> Action {
    ...
    let top_proc = sys.processes().values().max_by_key(|p| p.memory());
    ...
    Action {
        ...
        reversible: false,
        estimated_impact:
            "Immediately free significant memory, but may cause data loss in the killed app".into(),
    }
}
...
pub fn optimize_startup_action() -> Action {
    Action {
        ...
        description: "List all startup items and suggest which ones to disable for faster boot."
            .into(),
